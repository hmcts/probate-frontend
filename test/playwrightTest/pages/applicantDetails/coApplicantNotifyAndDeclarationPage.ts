...
export class CoApplicantNotifyAndDeclarationPage extends BasePage {
  ...
  async getIdList() {
    ...
    await this.page.goto(`${testConfig.TestE2EFrontendUrl}${testConfig.TestInviteIdListUrl}`, {
      waitUntil: 'load',
      timeout: 60000
    });
  ...
  async seeCoExecutorLaunchPage(idList) {
    ...
    await this.page.goto(`${testConfig.TestE2EFrontendUrl}${testConfig.TestInvitationUrl}/${idList}`, {
      waitUntil: 'load',
      timeout: 60000
    });
    await expect(this.page.locator('//*[@name="loginForm" or @id="main-content"]')).toBeEnabled();
    await this.page.goto(`${testConfig.TestE2EFrontendUrl}/pin`, {
      waitUntil: 'load',
      timeout: 60000
    });

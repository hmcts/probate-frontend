...
Data(TestConfigurator.createFeeInfoTableFor1Copy()).Scenario('Single Executor Journey - Fee Payment: Check can pay after cancel payment for no application fee', function (I, current) {
    data.copies.uk = current.noUKCopies;
    data.copies.overseas = current.noOverseasCopies;
    data.iht.grossValue = current.grossValue;
    data.iht.netValue = current.netValue;

...
class PaymentBreakdown extends Step {
    ...
    handleGet(ctx, formdata) {
        logger.info('handleGet method initiated for /payment-breakdown for case id: ' + (ctx.ccdCase.id !== null ? ctx.ccdCase.id : ''));
        ...
        'total fee: ' + ctx.total + ', for case id: ' + (ctx.ccdCase.id !== null ? ctx.ccdCase.id : ''));
    ...
    createCopiesLayout(formdata) {
        ...
        ' respectively, for case id: ' + (formdata.ccdCase.id !== null ? formdata.ccdCase.id : ''));
        return {
    ...
    * handlePost(ctx, errors, formdata, session, hostname) {
        logger.info('handlePost method initiated for /payment-breakdown for case id: ' + (ctx.ccdCase.id !== null ? ctx.ccdCase.id : ''));
        try {
            ...
            if (ctx.total > 0 && canCreatePayment) {
                ...
                (ctx.ccdCase.id !== null ? ctx.ccdCase.id : ''));
            } else {

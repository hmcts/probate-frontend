...
class SetupSecrets {
    setupSecrets() {
        ...
        if (process.env.NODE_ENV === 'dev-aat') {
            this.setLocalSecret('idam-s2s-secret', 'services.idam.service_key');
            this.setLocalSecret('ccidam-idam-api-secrets-probate', 'services.idam.probate_oauth2_secret');
            this.setLocalSecret('launchdarkly-key', 'featureToggles.launchDarklyKey');
            this.setLocalSecret('launchdarklyUserkeyFrontend', 'featureToggles.launchDarklyUser.key');
            this.setLocalSecret('payCaseWorkerUser', 'services.idam.probate_user_email');
            this.setLocalSecret('payCaseWorkerPass', 'services.idam.probate_user_password');
            this.setLocalSecret('postcode-service-token2', 'services.postcode.token');
        }
    }
    ...
    setLocalSecret(secretName, configPath) {
